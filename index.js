...
const sendReminderEmail2 = async (user, book) => {
  ...
  const mailOptions = {
    ...
    text: `Dear ${user.username},\n\n` + // Email body with user's username
          `This is a reminder that you have not returned the book "${book.title}".\n\n` + // Include book title and due date
...
app.post('/add-to-cart', async (req, res) => {
  ...
  dueDate.setMinutes(issueDate.getMinutes() + 1); // Set due date to 1 minutes after issue date
  ...
  try {
    ...
    if (user) {
      ...
      if (book && book.count > 0) { // Check if the book exists and is in stock
        ...
        // Schedule email reminder after 1 minutes using cron job
        const reminderTime = new Date();
        reminderTime.setMinutes(reminderTime.getMinutes() + 1); // Schedule reminder 1 minutes from now
...
app.post('/submit-review', async (req, res) => {
  const { email, username, title, review, rating } = req.body;
